...
namespace shenlan {
    namespace control {
        ...
        void StanleyController::ComputeControlCmd(const VehicleState &vehicle_state, const TrajectoryData &planning_published_trajectory, ControlCmd &cmd) 
        {
            ...
            e_y = std::atan2(this->k_y_ * e_y, current_vehicle_velocity + 1.0 ); 
            ...
            {
                kappa_factor_angle = -current_closest_point.kappa; // ，LGSVL中的车辆右转的时候，参考路径的曲率为负值
            }
            
            double raw_steering_control = e_y + e_theta_pd + kappa_factor_angle;
